...
//______________________________________________________________
    ...
    raV = float(w) / float(h); //Modifica ra del window segun ra del vp
    ra = raV;
    ...
    //Version con la esfera para evitar deformaciones
    raV = float(w)/float(h);
    ra = raV;
    if(raV > 1.0) {
...
void MyGLWidget::projectTransform() {
    ...
    //Vista Ortogonal
    //glm::mat4 Proj = glm::ortho(-radiScnTotal, radiScnTotal, -radiScnTotal, radiScnTotal, zNear, zFar);
